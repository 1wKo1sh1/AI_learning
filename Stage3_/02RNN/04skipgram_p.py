...
class SkipGramModel(nn.Module):
    ...
    def forward(self, centor):
        # context.shape -> (bs=4, 2,2)定义banchsize为4，每个bs内的数据为2*2形状（2行上下文，同时嵌入维度为2）
        centor_emb =  self.embedding[centor]  # 取出嵌入矩阵的对应参数
        y = self.linear(centor_emb)
        return y
...
for epoch in range(1, epochs + 1):
    ...
    for batch_index in range(0, len(context), bs):
        # 上下文的tensor
        context_tensor = torch.tensor(context[batch_index: batch_index + bs]) # context (4 2)
        # 中心词的tensor
        center_tensor = torch.tensor(center[batch_index: batch_index + bs])# center (4, )
        center_tensor = center_tensor.view(bs,1)

        output = model(center_tensor)# 4 1 14
        output = output.repeat(1, context_tensor.shape[1], 1) # 4 2 14
        output = output.view(-1, 14)   #8 14
        loss = criterion(output, context_tensor.view(-1))
        total_loss += loss
